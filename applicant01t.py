...
# ========== إعدادات Testnet ==========
BINANCE_TESTNET = True  # True للtestnet, False للواقعي
...
TRADING_SETTINGS = {
    ...
    'max_simultaneous_trades': 2,  # Max 2 trade at same time
...
class TelegramBotManager:
    ...
    def register_handlers(self):
        ...
        def send_welcome(message):
            ...
            welcome_text = f"""
🤖 <b>بوت التداول الآلي - {'TESTNET' if BINANCE_TESTNET else 'MAINNET'}</b>
...
اكتب أي أمر للبدء 🚀
        ...
        def status_command(message):
            ...
            try:
                ...
                status_text = f"""
📊 <b>حالة البوت الشاملة - {'TESTNET' if BINANCE_TESTNET else 'MAINNET'}</b>
   ...
   المعرف: {trade_id[-8:]}
        ...
        def cleanup_command(message):
            ...
            try:
                ...
                response_text = f"""
🧹 <b>نتيجة التنظيف الشامل - {'TESTNET' if BINANCE_TESTNET else 'MAINNET'}</b>
...
تمت العملية بنجاح ✅
        ...
        def pending_cleanup_command(message):
            ...
            try:
                ...
                response_text = f"""
🔍 <b>تنظيف الصفقات المعلقة في Binance - {'TESTNET' if BINANCE_TESTNET else 'MAINNET'}</b>
...
تم التنظيف ✅
        ...
        def close_all_command(message):
            ...
            try:
                ...
                # طلب تأكيد
                confirm_text = f"""
⚠️ <b>تأكيد الإغلاق الجماعي - {'TESTNET' if BINANCE_TESTNET else 'MAINNET'}</b>
...
هل تريد حقاً إغلاق جميع الصفقات؟
        ...
        def close_symbol_command(message):
            ...
            try:
                ...
                confirm_text = f"""
⚠️ <b>تأكيد إغلاق صفقات {symbol} - {'TESTNET' if BINANCE_TESTNET else 'MAINNET'}</b>
...
هل تريد إغلاق هذه الصفقات؟
        ...
        def sync_command(message):
            ...
            try:
                ...
                response_text = f"""
🔄 <b>نتيجة المزامنة الكاملة - {'TESTNET' if BINANCE_TESTNET else 'MAINNET'}</b>
...
<b>تفاصيل المواقع في Binance:</b>
        ...
        def balance_command(message):
            ...
            try:
                ...
                balance_text = f"""
💰 <b>حالة الرصيد والحساب - {'TESTNET' if BINANCE_TESTNET else 'MAINNET'}</b>
...
🕒 الوقت: {datetime.now(damascus_tz).strftime('%H:%M:%S')}
        ...
        def trades_command(message):
            ...
            try:
                ...
                trades_text = f"""
📈 <b>الصفقات النشطة ({len(active_trades)}) - {'TESTNET' if BINANCE_TESTNET else 'MAINNET'}</b>
   ...
   📅 البدء: {trade['timestamp'].strftime('%H:%M')}
        ...
        def positions_command(message):
            ...
            try:
                ...
                positions_text = f"""
📊 <b>المواقع المفتوحة في Binance ({len(active_positions)}) - {'TESTNET' if BINANCE_TESTNET else 'MAINNET'}</b>
   ...
   🎯 الهامش: ${float(position['initialMargin']):.4f}
        ...
        def history_command(message):
            ...
            try:
                ...
                history_text = f"""
📋 <b>آخر {len(recent_history)} صفقة مغلقة - {'TESTNET' if BINANCE_TESTNET else 'MAINNET'}</b>
...
🔹 {trade['symbol']} ({trade['side']})
   ...
   📅 الإغلاق: {trade.get('close_time', trade['timestamp']).strftime('%H:%M')}
        ...
        def settings_command(message):
            ...
            try:
                settings_text = f"""
⚙️ <b>إعدادات التداول الحالية - {'TESTNET' if BINANCE_TESTNET else 'MAINNET'}</b>
...
📈 <b>مستويات التداول:</b>
• LEVEL_1 (25-50%): تخصيص 50%
• LEVEL_2 (51-65%): تخصيص 75%  
• LEVEL_3 (66-100%): تخصيص 99%
...
🕒 آخر تحديث: {datetime.now(damascus_tz).strftime('%H:%M:%S')}
        ...
        def symbols_command(message):
            ...
            symbols_text = f"""
🎯 <b>العملات المدعومة ({len(TRADING_SETTINGS['symbols'])}) - {'TESTNET' if BINANCE_TESTNET else 'MAINNET'}</b>
...
🕒 الوقت: {datetime.now(damascus_tz).strftime('%H:%M:%S')}
        ...
        def signals_command(message):
            ...
            try:
                ...
                signals_text = f"""
📨 <b>آخر {len(recent_signals)} إشارة مستلمة - {'TESTNET' if BINANCE_TESTNET else 'MAINNET'}</b>
   ...
   ⏰ الوقت: {signal.get('received_time', datetime.now(damascus_tz)).strftime('%H:%M')}
        ...
        def force_close_command(message):
            ...
            try:
                ...
                result_text = f"""
🔒 <b>نتيجة الإغلاق الإجباري لـ {symbol} - {'TESTNET' if BINANCE_TESTNET else 'MAINNET'}</b>
...
تمت العملية 🎯
        ...
        def check_symbol_command(message):
            ...
            try:
                ...
                check_text = f"""
🔍 <b>فحص حالة {symbol} - {'TESTNET' if BINANCE_TESTNET else 'MAINNET'}</b>
...
<b>الصفقات النشطة:</b>
    ...
    def execute_close_all(self, message):
        ...
        try:
            ...
            result_text = f"""
🔒 <b>نتيجة الإغلاق الجماعي - {'TESTNET' if BINANCE_TESTNET else 'MAINNET'}</b>
...
📊 الإجمالي: {len(active_trades)} صفقة
    ...
    def execute_close_symbol(self, message, symbol):
        ...
        try:
            ...
            result_text = f"""
🔒 <b>نتيجة إغلاق صفقات {symbol} - {'TESTNET' if BINANCE_TESTNET else 'MAINNET'}</b>
...
تمت العملية 🎯
    ...
    def start_polling(self):
        ...
        try:
            logger.info(f"🤖 بدء تشغيل بوت Telegram للأوامر - {'TESTNET' if BINANCE_TESTNET else 'MAINNET'}...")
...
class MultiLevelTradeExecutor:
        ...
        try:
            ...
            if pending_trades:
                ...
                if self.notifier and pending_trades:
                    message = (
                        f"🧹 <b>تنظيف الصفقات المعلقة - {'TESTNET' if BINANCE_TESTNET else 'MAINNET'}</b>\n"
    ...
    def execute_trade(self, signal_data):
        ...
        try:
            ...
            if not trade_level:
                logger.error(f"❌ درجة ثقة غير كافية: {confidence_score} - الحد الأدنى 25 نقطة")
                return False, f"درجة ثقة غير كافية: {confidence_score}% - الحد الأدنى 25%"
            ...
            if order and order.get('orderId'):
                ...
                if self.notifier:
                    message = (
                        f"✅ <b>تم تنفيذ صفقة جديدة - المستوى {trade_level} - {'TESTNET' if BINANCE_TESTNET else 'MAINNET'}</b>\n"
                        f"العملة: {symbol}\n"
                        f"الاتجاه: {direction}\n"
        ...
        try:
            ...
            if order and order.get('orderId'):
                ...
                if self.notifier:
                    pnl_emoji = "🟢" if pnl_pct > 0 else "🔴"
                    message = (
                        f"🔒 <b>إغلاق صفقة - المستوى {trade['trade_level']} - {'TESTNET' if BINANCE_TESTNET else 'MAINNET'}</b>\n"
                        f"العملة: {symbol}\n"
                        f"الاتجاه: {direction}\n"
...
class SimpleSignalReceiver:
    ...
    def _validate_signal(self, signal_data):
        ...
        if confidence_score < 25:
            logger.error(f"❌ درجة ثقة غير كافية: {confidence_score}% - الحد الأدنى 25%")
            return False
...
def convert_signal_format(signal_data):
    ...
    try:
        ...
        if confidence_score < 25:
            logger.error(f"❌ درجة ثقة غير كافية في التحويل: {confidence_score}% - الحد الأدنى 25%")
            return None
...
def create_signal_notification(signal_data, success, message):
    ...
    try:
        ...
        # تحديد مستوى التداول
        trade_level = "LEVEL_1" if 25 <= confidence <= 50 else "LEVEL_2" if 51 <= confidence <= 65 else "LEVEL_3" if confidence >= 66 else "غير مؤهل"
        
        notification = (
            f"📡 <b>استقبال إشارة تداول - {'TESTNET' if BINANCE_TESTNET else 'MAINNET'}</b>\n"
...
class SimpleTradeBot:
    ...
    def __init__(self):
        ...
        # تهيئة العميل مع testnet
        try:
            self.client = Client(
                self.api_key, 
                self.api_secret,
                testnet=BINANCE_TESTNET  # ✅ هذا هو التغيير الرئيسي
            )
        ...
        logger.info(f"✅ تم تهيئة البوت المنفذ متعدد المستويات بنجاح - {'TESTNET' if BINANCE_TESTNET else 'MAINNET'}")
    
    def test_connection(self):
        """اختبار الاتصال"""
        try:
            server_time = self.client.futures_time()
            network_type = "TESTNET" if BINANCE_TESTNET else "MAINNET"
            logger.info(f"✅ اتصال Binance {network_type} API نشط - وقت السيرفر: {server_time['serverTime']}")
            
            # التحقق من رصيد Testnet
            if BINANCE_TESTNET:
            ...
            network_type = "TESTNET" if BINANCE_TESTNET else "MAINNET"
            logger.error(f"❌ فشل الاتصال بـ Binance {network_type} API: {e}")
            raise
    ...
    def check_testnet_balance(self):
        ...
        try:
            ...
            logger.info(f"💰 رصيد Testnet: {balance:.2f} USDT")
            
            if balance < TRADING_SETTINGS['min_balance_required']:
                logger.warning(f"⚠️ رصيد Testnet منخفض: {balance:.2f} USDT")
                if self.notifier:
                    self.notifier.send_message(
                        f"⚠️ <b>تحذير رصيد Testnet</b>\n"
                        f"الرصيد الحالي: {balance:.2f} USDT\n"
                        f"الحد الأدنى المطلوب: {TRADING_SETTINGS['min_balance_required']} USDT\n"
                        f"يرجى تموين الحساب في testnet.binancefuture.com"
                    )
            return balance
        except Exception as e:
            logger.error(f"❌ خطأ في التحقق من رصيد Testnet: {e}")
            return 0
    ...
    def get_status(self):
        ...
        return {
            'status': 'running',
            'network': 'TESTNET' if BINANCE_TESTNET else 'MAINNET',
...
def receive_heartbeat():
    ...
    try:
        ...
        if bot.notifier:
            heartbeat_msg = (
                f"💓 <b>نبضة اتصال من البوت المرسل - {'TESTNET' if BINANCE_TESTNET else 'MAINNET'}</b>\n"
                f"المصدر: {source}\n"
...
def main():
    ...
    try:
        ...
        logger.info(f"🚀 بدء تشغيل البوت المنفذ متعدد المستويات - {'TESTNET' if BINANCE_TESTNET else 'MAINNET'}...")
        
        # إرسال رسالة بدء التشغيل
        if bot.notifier:
            message = (
                f"🚀 <b>بدء تشغيل البوت المنفذ متعدد المستويات v3.0 - {'TESTNET' if BINANCE_TESTNET else 'MAINNET'}</b>\n"
                ...
                f"المستويات:\n"
                f"• LEVEL_1 (25-50%): تخصيص 50%\n"
                f"• LEVEL_2 (51-65%): تخصيص 75%\n"
                f"• LEVEL_3 (66-100%): تخصيص 99%\n"
