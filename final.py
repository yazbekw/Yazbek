...
class ScalpingTradingBot:
    ...
    TRADING_SETTINGS = {
        'symbols': ["ETHUSDT","SOLUSDT"],  # العملات المضافة
        'symbol_settings': {
        'ETHUSDT': {'used_balance': 8, 'leverage': 5, 'nominal_trade_size': 40},  # ✅
        'SOLUSDT': {'used_balance': 8, 'leverage': 5, 'nominal_trade_size': 40},  # ✅   
        },
        ...
        'stop_loss_pct': 0.20,
        'max_daily_trades': 30,  # زيادة الحد اليومي
    ...
    def send_performance_report(self):
        ...
        symbol_stats_text = ""
        for symbol in self.TRADING_SETTINGS['symbols']:
            stats = self.performance_stats['symbol_stats'][symbol]
            symbol_win_rate = (stats['wins'] / stats['trades'] * 100) if stats['trades'] > 0 else 0
            symbol_stats_text += f"{symbol}: {stats['trades']} صفقات | {symbol_win_rate:.1f}% فوز\n"
