...
class BNBScalpingBot:
    def __init__(self):
        ...
        self.request_delay = 2  # ثانيتين بين الطلبات
        self.analysis_interval = 120  # 120 ثانية بين التحليلات
        self.health_check_interval = 600  # 10 دقائق بين الفحوصات الصحية
        ...
        logging.info(f"معدل الطلبات: تأخير {self.request_delay} ثانية بين الطلبات")

    async def safe_api_call(self, func, *args, max_retries=3):
        """استدعاء API مع إعادة المحاولة وتجنب تجاوز الحدود"""
        for attempt in range(max_retries):
            try:
                # التأكد من مرور الوقت الكافي بين الطلبات
                ...
                if e.code == -1003:  # IP banned - too many requests
                    wait_time = min(300, 60 * (2 ** attempt))  # exponential backoff
                    warning_msg = f"⏳ تجاوز حد الطلبات، انتظار {wait_time} ثانية قبل إعادة المحاولة {attempt + 1}/{max_retries}"
                    logging.warning(warning_msg)
                    if self.telegram_enabled:
                        await self.send_telegram_notification(warning_msg, "warning")
                    await asyncio.sleep(wait_time)
                elif e.code == -1001:  # Internal error
                    logging.warning(f"خطأ داخلي في Binance، إعادة المحاولة {attempt + 1}/{max_retries}")
                    await asyncio.sleep(5)
                else:
                    raise e
            except Exception as e:
                logging.error(f"خطأ في استدعاء API: {e}")
                if attempt < max_retries - 1:
                    await asyncio.sleep(5)
                else:
                    raise e
        
        raise Exception("فشل بعد الحد الأقصى لمحاولات إعادة الاتصال")
    ...
    def get_futures_precision(self, symbol):
        """الحصول على معلومات الدقة من Binance"""
        try:
    ...
    async def get_websocket_price(self):
        """الحصول على السعر عبر WebSocket لتجنب طلبات HTTP المتكررة"""
        uri = f"wss://fstream.binance.com/ws/{self.symbol.lower()}@ticker"
        try:
            ...
            logging.warning("⏰ انتهت مهلة WebSocket، العودة إلى HTTP")
            self.websocket_connected = False
        except Exception as e:
            logging.warning(f"🔌 خطأ في WebSocket: {e}، العودة إلى HTTP")
            self.websocket_connected = False
        
        # Fallback إلى HTTP إذا فشل WebSocket
        return self.get_current_price_http()

    def get_current_price_http(self):
        """الحصول على السعر الحالي عبر HTTP (fallback)"""
        try:
            if self.client is None:
            ...
            logging.error(f"Error getting current price via HTTP: {e}")
            return 0

    async def get_current_price(self):
        """الحصول على السعر الحالي مع优先ية لـ WebSocket"""
        try:
            # محاولة WebSocket أولاً
            if not self.websocket_connected:
                price = await self.get_websocket_price()
                if price > 0:
                    return price
            
            # Fallback إلى HTTP
            price = self.get_current_price_http()
            if price > 0:
                return price
                
            return 0
        except Exception as e:
            logging.error(f"Error in get_current_price: {e}")
            return 0

    async def initialize(self):
        """تهيئة الاتصالات"""
        try:
            # التحقق من وجود متغيرات Binance البيئية
            ...
            try:
                await self.safe_api_call(
                    lambda: self.client.futures_change_leverage(symbol=self.symbol, leverage=self.leverage)
                )
                if self.telegram_enabled:
            ...
            try:
                account_info = await self.safe_api_call(
...
• **WebSocket:** {'🟢 مفعل' if self.websocket_connected else '🔴 HTTP'} 🌐
• **معدل الطلبات:** {self.request_delay} ثانية ⏱️
• **زمن التشغيل:** {datetime.now(self.damascus_tz).strftime('%Y-%m-%d %H:%M:%S')} ⏰
    ...
    async def execute_trade(self, signal_type, price):
        ...
        try:
            ...
            # حساب الكمية باستخدام الطريقة الآمنة
    ...
    async def monitor_position(self):
        ...
        while self.open_position and self.is_running:
            try:
                ...
                if time_in_position.total_seconds() > 15 * 60:
                    should_close = True
...
• **المدة:** {str(time_in_position).split('.')[0]} ⏱️
    ...
    async def health_check(self):
        ...
        try:
            ...
            await self.safe_api_call(lambda: self.client.futures_exchange_info())
            ...
            # التحقق من الرصيد
            account_info = await self.safe_api_call(lambda: self.client.futures_account())
            available_balance = float(account_info['availableBalance'])
            total_wallet_balance = float(account_info['totalWalletBalance'])
            
            # إرسال تقرير صحي كل 6 ساعات
            if self.health_check_counter % 72 == 0:
                status_message = f"""
🏥 **فحص صحي للبوت:** 🏥
• **اتصال Binance:** ✅ متصل
• **اتصال Telegram:** {'✅ متصل' if self.telegram_enabled else '🔴 غير مفعل'}  
• **اتصال WebSocket:** {'✅ متصل' if self.websocket_connected else '🔴 HTTP'} 
• **الرصيد الإجمالي:** {total_wallet_balance:.2f} USDT 💰
• **الرصيد المتاح:** {available_balance:.2f} USDT 💵
    ...
    async def run_bot(self):
        ...
        try:
            while self.is_running:
                ...
                # تحليل الإشارات كل 120 ثانية فقط
