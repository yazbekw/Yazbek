...
def restart_bot():
    ...
    return {'status': 'restarting', 'timestamp': datetime.now(damascus_tz).isoformat()}

@app.route('/keep-alive')
def keep_alive():
    """نقطة لإبقاء البوت نشطاً"""
    return {'status': 'alive', 'timestamp': datetime.now(damascus_tz).isoformat()}
...
class ScalpingTradingBot:
    ...
    def start_flask_server(self):
        ...
        flask_thread.start()
        logger.info("🚀 خادم Flask يعمل على المنفذ 10000")
