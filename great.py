...
TRADING_SETTINGS = {
    ...
    'base_trade_amount': 5,  # 5 دولار
    'leverage': 20,  # 20x رافعة
    'position_size': 5 * 20,  # 100 دولار حجم المركز
    'max_simultaneous_trades': 2,  # أقصى 2 صفقة في نفس الوقت
    'max_trades_per_symbol': 1,  # صفقة واحدة فقط لكل عملة
    'min_balance_required': 10,  # أقل رصيد مطلوب
}

# مستويات جني الأرباح حسب قوة الإشارة
TAKE_PROFIT_LEVELS = {
    'LEVEL_1': {  # إشارة متوسطة (41-65 نقطة)
...
class MultiLevelTradeExecutor:
    ...
    def get_trade_level(self, confidence_score):
        """تحديد مستوى التداول بناء على درجة الثقة"""
        if confidence_score >= 81:
            return 'LEVEL_3'
        elif confidence_score >= 66:
            return 'LEVEL_2'
        elif confidence_score >= 41:
            return 'LEVEL_1'
        else:
            return None
    ...
    def execute_trade(self, signal_data):
        ...
        try:
            ...
            if not trade_level:
                logger.error(f"❌ درجة ثقة غير كافية: {confidence_score}")
                return False, f"درجة ثقة غير كافية: {confidence_score}"
...
class SimpleSignalReceiver:
    ...
    def _validate_signal(self, signal_data):
        ...
        confidence_score = signal_data['confidence_score']
        if confidence_score < 41:
            logger.error(f"❌ درجة ثقة غير كافية: {confidence_score}% - الحد الأدنى 41%")
            return False
...
def convert_signal_format(signal_data):
    """تحويل تنسيق الإشارة من البوت المرسل إلى البوت المنفذ"""
    try:
...
def create_signal_notification(signal_data, success, message):
    ...
    try:
        ...
        # تحديد مستوى التداول
        trade_level = "LEVEL_1" if 41 <= confidence <= 65 else "LEVEL_2" if 66 <= confidence <= 80 else "LEVEL_3" if confidence >= 81 else "غير مؤهل"
    ...
    try:
        ...
        if bot.notifier:
            message = (
                ...
                f"المستويات:\n"
                f"• LEVEL_1 (41-65%): {TAKE_PROFIT_LEVELS['LEVEL_1']['profit_target']*1000:.1f} بالألف - تخصيص {TAKE_PROFIT_LEVELS['LEVEL_1']['allocation']*100}%\n"
