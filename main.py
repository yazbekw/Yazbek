...
class FuturesTradingBot:
    ...
    def get_historical_data(self, symbol, interval='30m', limit=100):
        """جلب البيانات التاريخية مع فحص الجودة ومحاولات متعددة"""
        max_retries = 3
        for attempt in range(max_retries):
            try:
                logger.info(f"🔍 محاولة {attempt + 1} لجلب بيانات {symbol} ({interval})")
                
                klines = self.client.futures_klines(symbol=symbol, interval=interval, limit=limit)
                
                # فحص جودة البيانات الأساسية
                if not klines or len(klines) < 30:
                    logger.warning(f"⚠️ بيانات غير كافية لـ {symbol}: {len(klines) if klines else 0} نقطة")
                ...
                if data['close'].isnull().any() or (data['close'] <= 0).any():
                    logger.error(f"❌ بيانات غير صالحة لـ {symbol} - قيم صفرية أو NaN")
                    if attempt < max_retries - 1:
                        time.sleep(2)
                        continue
                    return None
                
                # فحص أن البيانات ليست ثابتة
                price_std = data['close'].std()
                if price_std < data['close'].mean() * 0.001:  # إذا كان الانحراف المعياري أقل من 0.1%
                    logger.warning(f"⚠️ بيانات {symbol} ثابتة جداً - تقلبات ضعيفة")
                    if attempt < max_retries - 1:
                        time.sleep(2)
                        continue
                
                # فحص أن هناك حركة سعرية حقيقية
                unique_prices = data['close'].nunique()
                if unique_prices < len(data) * 0.3:  # إذا كانت أقل من 30% من الأسعار فريدة
                    logger.warning(f"⚠️ بيانات {symbol} متكررة - {unique_prices}/{len(data)} أسعار فريدة")
                    if attempt < max_retries - 1:
                        time.sleep(2)
                        continue
                
                logger.info(f"✅ تم جلب {len(data)} نقطة بيانات لـ {symbol}")
                return data
                
            except Exception as e:
                logger.error(f"❌ محاولة {attempt + 1} فشلت لـ {symbol}: {e}")
                if attempt < max_retries - 1:
                    time.sleep(2 ** attempt)  # exponential backoff
                else:
                    logger.error(f"❌ فشل جميع محاولات جلب البيانات لـ {symbol}")
                    return None
    ...
    def advanced_analyze_symbol(self, symbol):
        """تحليل متقدم مع نظام fallback قوي ومحاولات متعددة"""
        logger.info(f"🔍 بدء التحليل المتقدم لـ {symbol}")
        
        # قائمة بالأطر الزمنية للتحليل (من الأعلى للأدنى)
        timeframes = ['1d', '4h', '1h', '30m']
        advanced_analysis = None
        
        for timeframe in timeframes:
            try:
                logger.info(f"🔄 محاولة التحليل باستخدام إطار {timeframe} لـ {symbol}")
                ...
                if data is not None and len(data) >= 30:
                    ...
                    # إذا كان التحليل ناجحاً وذو ثقة مقبولة، نوقف المحاولات
                    if (advanced_analysis.get('phase') != 'غير محدد' and 
                        advanced_analysis.get('confidence', 0) > 0.2):
                        logger.info(f"✅ تحليل ناجح لـ {symbol} باستخدام {timeframe}")
                        break
                    else:
                        logger.warning(f"⚠️ تحليل ضعيف لـ {symbol} باستخدام {timeframe} - الثقة: {advanced_analysis.get('confidence', 0)}")
                else:
                    logger.warning(f"⚠️ بيانات غير كافية لـ {symbol} في إطار {timeframe}")
                    
            except Exception as e:
                logger.error(f"❌ خطأ في تحليل {symbol} باستخدام {timeframe}: {e}")
                continue
        
        # إذا فشلت جميع المحاولات
        if advanced_analysis is None or advanced_analysis.get('phase') == 'غير محدد':
            logger.error(f"❌ فشل جميع محاولات التحليل المتقدم لـ {symbol}")
            advanced_analysis = {
                'phase': 'غير محدد',
                'confidence': 0,
        ...
        # دمج القرارات مع معالجة محسنة للتحليل الفاشل
        return self._combine_strategies_with_notifications(
    ...
    def _combine_strategies_with_notifications(self, symbol, has_traditional_signal, traditional_direction, 
        ...
        """دمج الاستراتيجيات مع معالجة أفضل للتحليل الفاشل"""
        
        market_phase = advanced_analysis.get('phase', 'غير محدد')
        phase_confidence = advanced_analysis.get('confidence', 0)
        
        # فحص إذا كان التحليل المتقدم فاشلاً تماماً
        is_advanced_failed = (market_phase == 'غير محدد' and phase_confidence == 0)
        
        if is_advanced_failed:
            logger.warning(f"⚠️ التحليل المتقدم فاشل لـ {symbol}، استخدام التقليدي فقط")
            
            if has_traditional_signal:
                # استخدام الإشارة التقليدية مع تحذير
                traditional_analysis['warning'] = 'التحليل المتقدم غير متوفر - استخدام التقليدي فقط'
                traditional_analysis['strategy_type'] = 'traditional_fallback'
                traditional_analysis['market_phase'] = 'غير محدد'
                traditional_analysis['phase_confidence'] = 0
                
                # إشعار باستخدام البديل
                if self.notifier:
                    self.notifier.send_message(
                        f"🔄 <b>استخدام التحليل التقليدي كبديل</b>\n"
                        f"العملة: {symbol}\n"
                        f"السبب: فشل التحليل المتقدم\n"
                        f"الاتجاه: {traditional_direction}\n"
        ...
        # إذا كان التحليل المتقدم ناجحاً ولكن بثقة منخفضة
        has_advanced_signal = market_phase in ['markup', 'markdown']
        advanced_direction = 'LONG' if market_phase == 'markup' else 'SHORT' if market_phase == 'markdown' else None
        ...
        # 2. ⚠️ إشارة تقليدية فقط مع تحليل متقدم ضعيف
        elif has_traditional_signal and (not has_advanced_signal or phase_confidence < 0.4):
            logger.info(f"🔄 إشارة تقليدية لـ {symbol} مع تحليل متقدم ضعيف")
            
            # تخفيف قوة الإشارة بسبب ضعف التحليل المتقدم
            base_strength = traditional_analysis.get('signal_strength', 50)
            adjusted_strength = base_strength * 0.8  # تخفيف 20%
            
            traditional_analysis['signal_strength'] = adjusted_strength
            traditional_analysis['strategy_type'] = 'traditional_weak_advanced'
            traditional_analysis['market_phase'] = market_phase
            traditional_analysis['phase_confidence'] = phase_confidence
            traditional_analysis['warning'] = f'تحليل متقدم ضعيف - الثقة: {phase_confidence:.1%}'
