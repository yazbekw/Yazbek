...
class AdvancedSignalGenerator:
    ...
    def generate_signal(self, symbol, data, current_price):
        ...
        try:
            ...
            # ✅ تصفية الرموز - فقط العملات المطلوبة
            allowed_symbols = ['ETHUSDT', 'ADAUSDT', 'DOTUSDT', 'LINKUSDT']
            if symbol not in allowed_symbols:
                logger.info(f"⏭️ تخطي {symbol} - غير مدرج في القائمة المسموحة")
                return None
...
class AdvancedTradingBot:
    ...
    TRADING_SETTINGS = {
        'symbols': ["ETHUSDT", "ADAUSDT", "DOTUSDT", "LINKUSDT"],  # العملات الأربعة المطلوبة فقط
        'base_trade_size': 20,
        'max_leverage': 5,
        'max_active_trades': 3,
        'data_interval': '30m',
        'rescan_interval_minutes': 15,
        'min_signal_confidence': 0.72,  # نفس العتبة للشراء والبيع
    ...
    def can_open_trade(self, symbol, direction):
        ...
        # ✅ التحقق من أن الرمز مسموح به
        if symbol not in self.TRADING_SETTINGS['symbols']:
            reasons.append("الرمز غير مسموح به")
    ...
    def scan_market(self):
        ...
        for symbol in self.TRADING_SETTINGS['symbols']:  # ✅ فقط العملات المطلوبة
            try:
                if self.trade_manager.is_symbol_trading(symbol):
                    continue
