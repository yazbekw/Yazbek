...
TRADING_SETTINGS = {
    ...
    'use_exchange_orders': True,  # ⬅️ إضافة جديدة: استخدام أوامر المنصة
    'order_retry_attempts': 3,    # ⬅️ عدد محاولات وضع الأوامر
}
...
class TradeManager:
        ...
        try:
            ...
            for position in positions:
                ...
                if quantity != 0:
                    ...
                    # البحث عن أوامر الجني والوقف لهذه الصفقة
                    take_profit_order = None
                    stop_loss_order = None
                    
                    for order in open_orders:
                        if order['symbol'] == symbol and order['reduceOnly']:
                    ...
                    if symbol not in self.active_trades:
                        ...
                        self.active_trades[symbol] = {
                            ...
                            'exchange_orders_placed': take_profit_order is not None and stop_loss_order is not None
                        }
                    else:
                        # تحديث حالة أوامر الجني والوقف
                        self.active_trades[symbol]['has_take_profit'] = take_profit_order is not None
                        self.active_trades[symbol]['has_stop_loss'] = stop_loss_order is not None
                        self.active_trades[symbol]['exchange_orders_placed'] = take_profit_order is not None and stop_loss_order is not None
    ...
    def get_symbol_trades_count(self, symbol):
        """الحصول على عدد الصفقات النشطة لعملة معينة"""
        count = 0
    ...
    def get_symbol_trades_direction(self, symbol):
        """الحصول على اتجاه الصفقات النشطة لعملة معينة"""
        directions = []
    ...
    def update_trade_orders_status(self, symbol, has_tp, has_sl):
        """تحديث حالة أوامر الصفقة"""
        if symbol in self.active_trades:
            self.active_trades[symbol]['has_take_profit'] = has_tp
            self.active_trades[symbol]['has_stop_loss'] = has_sl
            self.active_trades[symbol]['exchange_orders_placed'] = has_tp and has_sl
...
class ScalpingTradingBot:
    ...
    def __init__(self):
        ...
        logger.info("✅ تم تهيئة بوت السكالبينج بنجاح مع النظام المزدوج")
    ...
    def manage_active_trades(self):
        """إدارة الصفقات النشطة مع التتبع الداخلي"""
        try:
            ...
            for symbol, trade in active_trades.items():
                ...
                # التتبع الداخلي إذا لم تكن الأوامر على المنصة
                if not trade.get('exchange_orders_placed', False):
                    self.check_trade_exit_internal(symbol, trade)
                    
        except Exception as e:
            logger.error(f"❌ خطأ في إدارة الصفقات: {e}")

    def check_trade_exit_internal(self, symbol, trade):
        """التتبع الداخلي للصفقات"""
        try:
            ...
            logger.error(f"❌ خطأ في التتبع الداخلي للصفقة {symbol}: {e}")

    def retry_failed_orders(self):
        """إعادة محاولة وضع الأوامر الفاشلة"""
        try:
            active_trades = self.trade_manager.get_all_trades()
            ...
            for symbol, trade in active_trades.items():
                if not trade.get('exchange_orders_placed', False):
                    ...
                    if self.TRADING_SETTINGS['use_exchange_orders']:
                        self.place_exchange_orders_with_retry(symbol, trade)
                        
        except Exception as e:
            logger.error(f"❌ خطأ في إعادة محاولة الأوامر: {e}")
    ...
    def place_exchange_orders_with_retry(self, symbol, trade):
        """محاولة وضع أوامر المنصة مع إعادة المحاولة"""
        if not self.TRADING_SETTINGS['use_exchange_orders']:
            return False
            
        max_attempts = self.TRADING_SETTINGS['order_retry_attempts']
        
        for attempt in range(max_attempts):
            try:
                logger.info(f"🔄 محاولة {attempt + 1} لوضع أوامر المنصة لـ {symbol}")
                
                tp_success = self.place_take_profit_order(
                    symbol, trade['quantity'], trade['entry_price'], trade['side']
                )
                
                sl_success = self.place_stop_loss_order(
                    symbol, trade['quantity'], trade['entry_price'], trade['side']
                )
                
                if tp_success and sl_success:
                    self.trade_manager.update_trade_orders_status(symbol, True, True)
                    logger.info(f"✅ تم وضع أوامر المنصة بنجاح لـ {symbol}")
                    
                    if self.notifier:
                        message = (
                            f"✅ <b>تم وضع أوامر المنصة</b>\n"
                            f"العملة: {symbol}\n"
                            f"الاتجاه: {trade['side']}\n"
                            f"🎯 جني الربح: ${self.calculate_take_profit_price(trade['entry_price'], trade['side']):.4f}\n"
                            f"🛡️ وقف الخسارة: ${self.calculate_stop_loss_price(trade['entry_price'], trade['side']):.4f}\n"
                            f"الوقت: {datetime.now(damascus_tz).strftime('%H:%M:%S')}"
                        )
                        self.notifier.send_message(message)
                    return True
                else:
                    logger.warning(f"⚠️ فشل وضع الأوامر للمحاولة {attempt + 1} لـ {symbol}")
                    time.sleep(2)  # انتظار قبل المحاولة التالية
                    
            except Exception as e:
                logger.error(f"❌ خطأ في المحاولة {attempt + 1} لوضع الأوامر لـ {symbol}: {e}")
                time.sleep(2)
        
        logger.error(f"❌ فشل جميع محاولات وضع الأوامر لـ {symbol}")
        return False
    ...
    def send_startup_message(self):
        if self.notifier:
            ...
            message = (
                "⚡ <b>بدء تشغيل بوت السكالبينج بالنظام المزدوج</b>\n"
                f"الاستراتيجية: تقاطع المتوسطات + RSI + الماكد\n"
                ...
                f"أوامر المنصة: {'مفعلة' if self.TRADING_SETTINGS['use_exchange_orders'] else 'معطلة'}\n"
                f"محاولات الأوامر: {self.TRADING_SETTINGS['order_retry_attempts']}\n"
    ...
    def send_performance_report(self):
        ...
        # حساب الصفقات ذات الأوامر على المنصة
        trades_with_orders = 0
        active_trades_data = self.trade_manager.get_all_trades()
        for trade in active_trades_data.values():
            if trade.get('exchange_orders_placed', False):
                trades_with_orders += 1
        
        message = (
            f"📊 <b>تقرير أداء السكالبينج</b>\n"
            f"الاستراتيجية: تقاطع المتوسطات + RSI + الماكد\n"
            f"الصفقات النشطة: {active_trades}\n"
            f"الصفقات بأوامر منصة: {trades_with_orders}\n"
    ...
    def send_heartbeat(self):
        if self.notifier:
            ...
            message = f"💓 بوت السكالبينج نشط - الصفقات النشطة: {active_trades} | بأوامر منصة: {trades_with_orders}"
            self.notifier.send_message(message)
    ...
    def execute_trade(self, signal):
        try:
            ...
            if order and order['orderId']:
                ...
                # محاولة وضع أوامر المنصة
                exchange_orders_placed = False
                if self.TRADING_SETTINGS['use_exchange_orders']:
                    time.sleep(1)  # انتظار بسيط قبل وضع الأوامر
                    exchange_orders_placed = self.place_exchange_orders_with_retry(
                        symbol, {
                            'quantity': quantity,
                ...
                trade_data = {
                    ...
                    'has_take_profit': exchange_orders_placed,
                    'has_stop_loss': exchange_orders_placed,
                    'exchange_orders_placed': exchange_orders_placed
                }
                ...
                if self.notifier:
                    ...
                    order_status = "✅ على المنصة" if exchange_orders_placed else "🔄 تتبع داخلي"
    ...
    def force_close_trade(self, symbol, reason="إغلاق إجباري"):
        ...
        return self.close_trade(symbol, reason)
        ...
        return [
            {
                ...
                'exchange_orders_placed': trade.get('exchange_orders_placed', False)
            }
    ...
    def run(self):
        logger.info("🚀 بدء تشغيل بوت السكالبينج بالنظام المزدوج...")
