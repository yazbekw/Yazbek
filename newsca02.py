...
TRADING_SETTINGS = {
    ...
    'target_profit_pct': 0.30,  # جني ربح 0.15%
    'stop_loss_pct': 0.15,     # وقف خسارة 0.08%
...
class ScalpingTradingBot:
    ...
    def __init__(self):
        ...
        logger.info("✅ تم تهيئة بوت السكالبينج بنجاح مع أوامر الجني والوقف على المنصة")
            ...
            if symbol_info:
                ...
                if price_filter:
                    ...
                    while tick_size < 1:
                        ...
                        precision += 1
                    return precision
            return 4  # القيمة الافتراضية
        except Exception as e:
            logger.error(f"❌ خطأ في الحصول على دقة السعر: {e}")
            return 4

    def adjust_price_precision(self, symbol, price):
        """ضبط السعر حسب دقة العملة"""
        try:
            precision = self.get_symbol_price_precision(symbol)
            return round(price, precision)
        except Exception as e:
            logger.error(f"❌ خطأ في ضبط دقة السعر: {e}")
    ...
    def place_take_profit_order(self, symbol, quantity, entry_price, direction):
        ...
        try:
            ...
            take_profit_price = self.adjust_price_precision(symbol, take_profit_price)
            
            # التحقق من أن سعر الجني منطقي بالنسبة للسوق
            current_price = self.get_current_price(symbol)
            if direction == 'LONG' and take_profit_price <= current_price:
                take_profit_price = current_price * (1 + self.TRADING_SETTINGS['target_profit_pct'] / 100)
                take_profit_price = self.adjust_price_precision(symbol, take_profit_price)
            elif direction == 'SHORT' and take_profit_price >= current_price:
                take_profit_price = current_price * (1 - self.TRADING_SETTINGS['target_profit_pct'] / 100)
                take_profit_price = self.adjust_price_precision(symbol, take_profit_price)
            ...
            logger.info(f"✅ تم وضع أمر جني الربح لـ {symbol} على السعر: ${take_profit_price:.6f}")
            return True
            
        except Exception as e:
            logger.error(f"❌ فشل وضع أمر جني الربح لـ {symbol}: {e}")
            # محاولة بديلة باستخدام LIMIT order
            return self.place_take_profit_limit_order(symbol, quantity, entry_price, direction)

    def place_take_profit_limit_order(self, symbol, quantity, entry_price, direction):
        """محاولة بديلة لوضع أمر جني الربح باستخدام LIMIT"""
        try:
            take_profit_price = self.calculate_take_profit_price(entry_price, direction)
            take_profit_price = self.adjust_price_precision(symbol, take_profit_price)
            ...
            logger.info(f"✅ تم وضع أمر جني الربح (LIMIT) لـ {symbol} على السعر: ${take_profit_price:.6f}")
            return True
            
        except Exception as e:
            logger.error(f"❌ فشل وضع أمر جني الربح (LIMIT) لـ {symbol}: {e}")
            return False
    ...
    def place_stop_loss_order(self, symbol, quantity, entry_price, direction):
        ...
        try:
            ...
            stop_loss_price = self.adjust_price_precision(symbol, stop_loss_price)
            
            # التحقق من أن سعر الوقف منطقي بالنسبة للسوق
            current_price = self.get_current_price(symbol)
            if direction == 'LONG' and stop_loss_price >= current_price:
                stop_loss_price = current_price * (1 - self.TRADING_SETTINGS['stop_loss_pct'] / 100)
                stop_loss_price = self.adjust_price_precision(symbol, stop_loss_price)
            elif direction == 'SHORT' and stop_loss_price <= current_price:
                stop_loss_price = current_price * (1 + self.TRADING_SETTINGS['stop_loss_pct'] / 100)
                stop_loss_price = self.adjust_price_precision(symbol, stop_loss_price)
            ...
            logger.info(f"🛡️ تم وضع أمر وقف الخسارة لـ {symbol} على السعر: ${stop_loss_price:.6f}")
            return True
            
        except Exception as e:
            logger.error(f"❌ فشل وضع أمر وقف الخسارة لـ {symbol}: {e}")
            # محاولة بديلة باستخدام STOP order
            return self.place_stop_loss_limit_order(symbol, quantity, entry_price, direction)

    def place_stop_loss_limit_order(self, symbol, quantity, entry_price, direction):
        """محاولة بديلة لوضع أمر وقف الخسارة"""
        try:
            stop_loss_price = self.calculate_stop_loss_price(entry_price, direction)
            stop_loss_price = self.adjust_price_precision(symbol, stop_loss_price)
            
            # حساب سعر التنفيذ (أقل قليلاً من سعر الوقف للبيع، أعلى قليلاً للشراء)
            execution_price = stop_loss_price * 0.999 if direction == 'LONG' else stop_loss_price * 1.001
            execution_price = self.adjust_price_precision(symbol, execution_price)
            ...
            logger.info(f"🛡️ تم وضع أمر وقف الخسارة (STOP) لـ {symbol} على السعر: ${stop_loss_price:.6f}")
            return True
            
        except Exception as e:
            logger.error(f"❌ فشل وضع أمر وقف الخسارة (STOP) لـ {symbol}: {e}")
            return False
    ...
    def send_startup_message(self):
        if self.notifier:
            ...
            message = (
                "⚡ <b>بدء تشغيل بوت السكالبينج مع أوامر الجني والوقف</b>\n"
                f"الاستراتيجية: تقاطع المتوسطات + RSI + الماكد\n"
        ...
        try:
            ...
            if order and order['orderId']:
                ...
                if self.notifier:
                    ...
                    take_profit_price = self.calculate_take_profit_price(executed_price, direction)
                    take_profit_price = self.adjust_price_precision(symbol, take_profit_price)
                    stop_loss_price = self.calculate_stop_loss_price(executed_price, direction)
                    stop_loss_price = self.adjust_price_precision(symbol, stop_loss_price)
                    ...
                    message = (
                        ...
                        f"🎯 جني الربح: {self.TRADING_SETTINGS['target_profit_pct']}% (${take_profit_price:.6f})\n"
                        f"🛡️ وقف الخسارة: {self.TRADING_SETTINGS['stop_loss_pct']}% (${stop_loss_price:.6f})\n"
    ...
    def run(self):
        logger.info("🚀 بدء تشغيل بوت السكالبينج مع أوامر الجني والوقف...")
