...
TRADING_SETTINGS = {
    ...
    'btc_confirmation_required': True,  # إضافة مؤشر تأكيد البيتكوين
    'min_btc_confidence': 0.80,  # الحد الأدنى لثقة إشارة البيتكوين
}
...
class SimpleOrderManager:
    ...
    def calculate_stop_prices(self, symbol, direction, entry_price):
        """حساب أسعار الوقف والجني مع ضبط الدقة"""
        try:
            # حساب الأسعار الأساسية
            if direction == 'LONG':
                stop_loss_price = entry_price * (1 - TRADING_SETTINGS['stop_loss_pct'] / 100)
            ...
            # التأكد من أن الأسعار منطقية
            if direction == 'LONG':
                if stop_loss_price >= entry_price:
                    stop_loss_price = entry_price * 0.999  # أقل قليلاً من سعر الدخول
                if take_profit_price <= entry_price:
                    take_profit_price = entry_price * 1.001  # أعلى قليلاً من سعر الدخول
            else:
                if stop_loss_price <= entry_price:
                    stop_loss_price = entry_price * 1.001  # أعلى قليلاً من سعر الدخول
                if take_profit_price >= entry_price:
                    take_profit_price = entry_price * 0.999  # أقل قليلاً من سعر الدخول
            
            logger.info(f"📊 الأسعار المعدلة لـ {symbol}:")
            logger.info(f"  💰 الدخول: ${entry_price:.4f}")
            logger.info(f"  🎯 الجني: ${take_profit_price:.4f}")
            logger.info(f"  🛡️ الوقف: ${stop_loss_price:.4f}")
    ...
    def place_stop_orders(self, symbol, direction, quantity, entry_price):
        """وضع أوامر الوقف والجني مع معالجة الدقة"""
        try:
            logger.info(f"🔄 محاولة وضع أوامر الوقف لـ {symbol}...")
            
            # حساب الأسعار المعدلة
            ...
            # محاولة بديلة مع تقريب أكثر تحفظاً
    ...
    def _place_alternative_orders(self, symbol, direction, quantity, entry_price):
        """محاولة بديلة لوضع الأوامر مع تقريب أكثر تحفظاً"""
        try:
            logger.info(f"🔄 محاولة بديلة لوضع الأوامر لـ {symbol}...")
            
            # استخدام تقريب أبسط
            if direction == 'LONG':
                stop_loss_price = round(entry_price * 0.999, 2)  # تقريب لرقمين عشريين
                take_profit_price = round(entry_price * 1.002, 2)
            else:
                stop_loss_price = round(entry_price * 1.001, 2)
                take_profit_price = round(entry_price * 0.998, 2)
            
            adjusted_quantity = round(quantity, 6)  # تقريب الكمية
            
            # محاولة وضع الأوامر مع التقريب البسيط
            sl_order = self.client.futures_create_order(
                symbol=symbol,
                side='SELL' if direction == 'LONG' else 'BUY',
                type='STOP_MARKET',
...
class ScalpingTradingBot:
    ...
    def __init__(self):
        ...
        self.btc_confirmation = BTCConfirmationSignalGenerator()  # إضافة مؤشر تأكيد البيتكوين
        ...
        logger.info("✅ تم تهيئة بوت السكالبينج بنجاح مع مدير الدقة ومؤشر تأكيد البيتكوين")
        ...
        try:
            ...
            if order and order['orderId']:
                ...
                trade_data = {
                    ...
                    'btc_confirmation': btc_confirmation  # حفظ معلومات تأكيد البيتكوين
                }
    ...
    def run(self):
        logger.info("🚀 بدء تشغيل بوت السكالبينج مع نظام إدارة الدقة وتأكيد البيتكوين...")
