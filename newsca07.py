...
class SimpleOrderManager:
    ...
    def validate_stop_prices_against_current(self, symbol, direction, entry_price, take_profit_price, stop_loss_price):
        ...
        try:
            ...
            if direction == 'LONG':
                ...
                if current_to_stop_ratio >= 0.998:
    ...
    def place_stop_orders_with_validation(self, symbol, direction, quantity, entry_price):
        ...
        try:
            ...
            if not self.validate_stop_prices_against_current(symbol, direction, entry_price, take_profit_price, stop_loss_price):
                ...
                if direction == 'LONG':
                    take_profit_price = self.precision_manager.adjust_price(symbol, entry_price * 1.01)
                    stop_loss_price = self.precision_manager.adjust_price(symbol, entry_price * 0.99)
                else:
                    take_profit_price = self.precision_manager.adjust_price(symbol, entry_price * 0.99)
                    stop_loss_price = self.precision_manager.adjust_price(symbol, entry_price * 1.01)
    ...
    def sync_active_trades(self):
        """مزامنة المراكز النشطة مع المنصة - محسنة لإرسال إشعارات الإغلاق"""
        try:
            ...
            # التحقق من الصفقات المغلقة
            current_active_symbols = {position['symbol'] for position in positions if float(position['positionAmt']) != 0}
    ...
    def _handle_closed_position(self, symbol):
        """معالجة صفقة مغلقة مكتشفة - محسنة لإرسال الإشعارات"""
        if symbol in self.active_trades:
            trade = self.active_trades[symbol]
            
            # الحصول على السعر الحالي لحساب PnL
            current_price = self._get_current_price(symbol)
            ...
            logger.info(f"🔄 اكتشاف إغلاق صفقة: {symbol} - PnL: {pnl_pct:+.2f}%")
...
class ScalpingTradingBot:
    ...
    def check_expired_trades(self):
        """التحقق من الصفقات التي انتهت مدتها الزمنية - محسنة لإرسال الإشعارات"""
        try:
    ...
    def force_close_trade(self, symbol, reason="إغلاق إجباري"):
        """إغلاق الصفقة قسراً عند انتهاء المدة - محسنة لإرسال الإشعارات"""
        try:
