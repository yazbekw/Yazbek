...
TRADING_SETTINGS = {
    ...
    # 🆕 إعدادات محسنة للإشارة الأولى
    'first_trade_requirements': {
        'min_volume_ratio': 1.2,        # حجم أعلى من المتوسط بـ 20%
        'min_rsi_strength': 2,          # RSI أقوى ب 2 نقاط
        'min_candle_strength': 0.35,    # شمعة أقوى (35%)
        'confirmation_candles': 1       # عدد الشموع للتأكيد بعد التقاطع
    }
}
...
class MACDTrendManager:
    ...
    def start_new_trend(self, symbol, direction, signal_type, macd_status):
        ...
        self.active_trends[symbol] = {
            ...
            'first_trade_improved': True  # 🆕 علامة أن الصفقة الأولى محسنة
        }
...
class AdvancedMACDSignalGenerator:
    ...
    def _calculate_advanced_indicators(self, data):
        ...
        return {
            ...
            'current_high': latest['high'],  # 🆕 إضافة
            'current_low': latest['low'],    # 🆕 إضافة
    ...
    def _analyze_base_signal(self, indicators, symbol, current_price, macd_status, data):
        """تحليل الإشارة الأساسية مع شروط محسنة للصفقة الأولى"""
        ...
        candle_range = indicators['current_high'] - indicators['current_low']
        candle_strength = candle_body / (candle_range + 1e-10)
    
        strong_bullish_candle = is_bullish_candle and candle_strength > 0.3
        strong_bearish_candle = is_bearish_candle and candle_strength > 0.3
        ...
        candle_strength_condition = candle_strength > TRADING_SETTINGS['first_trade_requirements']['min_candle_strength']
    
        # 🟢 إشارة شراء أساسية محسنة
        if (ema9_cross_above_21 and 
            rsi_strength_condition_buy and  # 🆕 RSI أقوى
            macd_status['bullish'] and 
            strong_bullish_candle and
            volume_condition and            # 🆕 تأكيد حجم
            candle_strength_condition):     # 🆕 قوة شمعة
            ...
            return {
                ...
                'direction': 'LONG',
                'confidence': 0.95,
                'reason': f'تقاطع فعلي محسن - EMA 9 فوق EMA 21 + حجم عالي + شمعة قوية (قوة: {candle_strength:.1%})',
                ...
                'improved_signal': True  # 🆕 إشارة محسنة
            }
    
        # 🟢 إشارة بيع أساسية محسنة
        if (ema9_cross_below_21 and 
            rsi_strength_condition_sell and  # 🆕 RSI أقوى
            ...
            return {
                ...
                'direction': 'SHORT',
                'confidence': 0.95,
                'reason': f'تقاطع فعلي محسن - EMA 9 تحت EMA 21 + حجم عالي + شمعة قوية (قوة: {candle_strength:.1%})',
                'indicators': indicators,
                'timestamp': datetime.now(damascus_tz),
                'current_price': current_price,
                'signal_type': 'BASE_CROSSOVER',
        ...
        try:
            ...
            if bot:
                ...
                if data is not None:
                    ...
                    if should_exit:
                        logger.info(f"🔮 إغلاق مبكر بالماكد لـ {symbol}: {reason}")
...
class TelegramNotifier:
    ...
    def send_signal_alert(self, symbol, signal, current_price, trend_status=None):
        ...
        try:
            ...
            improved_marker = " 🚀" if signal.get('improved_signal', False) else ""
            ...
            message = (
                f"{direction_emoji} <b>إشارة تداول جديدة</b> {signal_type_emoji}{improved_marker}\n"
                f"العملة: {symbol}\n"
                ...
                f"• الحجم: {signal['indicators']['volume']:.0f} (المتوسط: {signal['indicators']['volume_avg']:.0f})\n"
                f"{macd_info}"
                f"{trend_info}"
                f"الوقت: {datetime.now(damascus_tz).strftime('%H:%M:%S')}"
            )
...
class AdvancedMACDTrendBot:
    ...
    def __init__(self):
        ...
        self.performance_stats = {
            ...
            'improved_signals_used': 0,  # 🆕 إحصائيات الإشارات المحسنة
        }
    ...
    def send_startup_message(self):
        ...
        if self.notifier:
            ...
            improved_settings = TRADING_SETTINGS['first_trade_requirements']
            
            message = (
                "🚀 <b>بدء تشغيل بوت الماكد المتقدم المحسن</b>\n"
                f"الاستراتيجية: EMA 9/21 + RSI 14 + MACD + نظام الترندات\n"
                f"🎯 <b>الإشارات المحسنة للصفقة الأولى:</b>\n"
                f"• نسبة الحجم: {improved_settings['min_volume_ratio']}x\n"
                f"• قوة RSI: +{improved_settings['min_rsi_strength']} نقطة\n"
                f"• قوة الشمعة: {improved_settings['min_candle_strength']:.0%}\n"
    ...
    def send_daily_report(self):
        ...
        improved_signals = self.performance_stats['improved_signals_used']
        
        message = (
            f"📊 <b>التقرير اليومي - بوت الماكد المتقدم المحسن</b>\n"
            ...
            f"🔮 <b>إحصائيات المحسنات:</b>\n"
            f"• إغلاق مبكر: {macd_exits}\n"
            f"• إشارات محسنة: {improved_signals}\n"
            f"═══════════════════\n"
            f"💰 <b>الرصيد:</b>\n"
    ...
    def send_performance_report(self):
        ...
        message = (
            f"📈 <b>تقرير أداء البوت المتقدم المحسن</b>\n"
            f"الصفقات النشطة: {active_trades}\n"
            f"الترندات النشطة: {active_trends}\n"
    ...
    def execute_trade(self, signal):
        ...
        try:
            ...
            # 🆕 تسجيل الإشارات المحسنة
            if signal.get('improved_signal', False):
                self.performance_stats['improved_signals_used'] += 1
    ...
    def run(self):
        ...
        logger.info("🚀 بدء تشغيل بوت الماكد المتقدم المحسن...")
