...
class AdvancedMACDSignalGenerator:
    ...
    def _calculate_advanced_indicators(self, data):
        ...
        return {
            ...
            'prev_open': prev['open'],
            'prev_high': prev['high'],  # 🆕 إضافة prev_high
            'prev_low': prev['low'],    # 🆕 إضافة prev_low
    ...
    def _analyze_base_signal(self, indicators, symbol, current_price, macd_status, data):
        ...
        if (ema9_cross_above_21 and 
            prev_candle_bullish and          # 🎯 شمعة سابقة صاعدة
            price_above_prev_mid and         # 🎯 السعر فوق منتصف الشمعة السابقة
        ...
        if (ema9_cross_below_21 and 
            prev_candle_bearish and          # 🎯 شمعة سابقة هابطة
            price_below_prev_mid and         # 🎯 السعر تحت منتصف الشمعة السابقة
...
class AdvancedMACDTrendBot:
    ...
    def execute_trade(self, signal):
        """تنفيذ الصفقة في نظام الماكد المتقدم - محسّن"""
        try:
            ...
            if order and order.get('orderId'):
                ...
                # ✅ التأكد من وجود المرجع
                if not hasattr(self.trade_manager, 'bot_instance') or not self.trade_manager.bot_instance:
