...
class BinanceFuturesAnalyzer:
    ...
    async def make_signed_request(self, endpoint: str, params: Dict = None) -> Dict:
    ...
    async def get_account_balance(self) -> List[Dict]:
        ...
        return [asset for asset in data if float(asset['balance']) != 0]

    async def get_positions(self) -> List[Dict]:
        """جلب المراكز المفتوحة"""
        endpoint = '/fapi/v2/positionRisk'
        return await self.make_signed_request(endpoint)

    async def get_income_history(self, symbol: str = None, limit: int = 1000) -> List[Dict]:
        """جلب سجل الدخل (الصفقات، العمولات، التمويل)"""
        endpoint = '/fapi/v1/income'
        params = {'limit': limit}
        if symbol:
            params['symbol'] = symbol
            
        return await self.make_signed_request(endpoint, params)

    async def get_user_trades(self, symbol: str = None, limit: int = 1000) -> List[Dict]:
        """جلب سجل الصفقات"""
    ...
    async def get_all_symbols(self) -> List[str]:
        """جلب جميع الرموز المتاحة في العقود الآجلة"""
    ...
    async def get_all_trades_data(self) -> pd.DataFrame:
        """جلب جميع بيانات الصفقات من الحساب"""
        print("🔄 جاري جمع بيانات الصفقات من العقود الآجلة...")
        
        all_trades = []
        
        # جلب المراكز النشطة أولاً
        positions = await self.get_positions()
        active_symbols = [pos['symbol'] for pos in positions if float(pos['positionAmt']) != 0]
        
        # جلب جميع الرموز التي لها صفقات
        symbols = await self.get_all_symbols()
        
        # نحدد الرموز للبحث (المراكز النشطة + عينات أخرى)
        target_symbols = active_symbols + symbols[:20]  # تحديد للكفاءة
        
        for symbol in target_symbols:
            try:
                print(f"📊 جاري جمع صفقات {symbol}...")
                trades = await self.get_user_trades(symbol, limit=500)
                if trades:
                    for trade in trades:
                        trade['symbol'] = symbol
    ...
    def analyze_futures_trades(self, trades_df: pd.DataFrame) -> pd.DataFrame:
        ...
        trades_df['commission'] = pd.to_numeric(trades_df['commission'])
        trades_df['realizedPnl'] = pd.to_numeric(trades_df.get('realizedPnl', 0))
        
        # تحديد نوع الصفقة (شراء/بيع)
        trades_df['side'] = trades_df['side']
        trades_df['position_side'] = trades_df.get('positionSide', 'BOTH')
    ...
    def calculate_futures_pnl(self, trades_df: pd.DataFrame) -> pd.DataFrame:
        ...
        for symbol, group in symbol_groups:
            ...
            long_trades = group[group['position_side'] == 'LONG']
            short_trades = group[group['position_side'] == 'SHORT']
            both_trades = group[group['position_side'] == 'BOTH']
    ...
    def create_futures_summary_plot(self, pnl_df: pd.DataFrame, positions_df: pd.DataFrame) -> str:
        ...
        if not pnl_df.empty:
            ...
            plt.subplot(2, 3, 1)
            profitable_df = pnl_df[pnl_df['net_pnl'] > 0]
            losing_df = pnl_df[pnl_df['net_pnl'] < 0]
            
            if not profitable_df.empty:
                plt.bar(profitable_df['symbol'], profitable_df['net_pnl'], color='#2ecc71', label='ربح')
            if not losing_df.empty:
                plt.bar(losing_df['symbol'], losing_df['net_pnl'], color='#e74c3c', label='خسارة')
            ...
            # حجم التداول
            plt.subplot(2, 3, 3)
            plt.bar(pnl_df['symbol'], pnl_df['total_volume'], color='#f39c12', alpha=0.7)
            plt.title('حجم التداول ($)', fontsize=12, fontweight='bold')
            plt.xticks(rotation=45)
        
        if not positions_df.empty:
            # المراكز الحالية
            plt.subplot(2, 3, 4)
            colors = ['#2ecc71' if x > 0 else '#e74c3c' for x in positions_df['unrealized_pnl']]
            plt.bar(positions_df['symbol'], positions_df['unrealized_pnl'], color=colors)
            ...
            plt.bar(positions_df['symbol'], positions_df['position_amt'].abs(), color=colors_side)
    ...
    async def generate_full_futures_report(self):
        ...
        try:
            print("🔄 جاري جمع بيانات العقود الآجلة...")
            
            # جلب بيانات الصفقات
            trades_df = await self.get_all_trades_data()
