...
class BinanceFuturesReport:
    def __init__(self):
        ...
        try:
            ...
            print("✅ تم تهيئة عميل Binance Testnet بنجاح")
    ...
    def get_account_info(self):
        """الحصول على معلومات الحساب - التصحيح"""
        try:
    ...
    def get_positions(self):
        ...
        try:
            ...
            for position in positions:
                ...
                if position_amt != 0:
                    # ⭐⭐ التصحيح: استخدام 'unrealizedProfit' بدلاً من 'unRealizedProfit'
                    active_positions.append({
                        'symbol': position['symbol'],
                        'position_amt': position_amt,
                        'entry_price': float(position['entryPrice']),
                        'unrealized_pnl': float(position['unrealizedProfit']),  # ⭐ التصحيح هنا
                        'leverage': int(position['leverage']),
                        'direction': 'LONG' if position_amt > 0 else 'SHORT',
                        'liquidation_price': float(position.get('liquidationPrice', 0))
                    })
            
            print(f"✅ تم العثور على {len(active_positions)} مركز نشط")
    ...
    def get_user_trades(self, symbol, limit=500):
        """الحصول على الصفقات - التصحيح"""
        try:
            # ⭐⭐ التصحيح: اسم الدالة الصحيح هو futures_user_trades
            trades = self.client.futures_account_trades(symbol=symbol, limit=limit)  # ⭐ التصحيح هنا
            print(f"✅ تم جلب {len(trades)} صفقة لـ {symbol}")
            return trades
        except Exception as e:
    ...
    def get_income_history(self, symbol=None, limit=100):
        """الحصول على سجل الدخل (عمولات، تمويل، إلخ)"""
        try:
            if symbol:
                income = self.client.futures_income_history(symbol=symbol, limit=limit)
            else:
                income = self.client.futures_income_history(limit=limit)
            return income
        except Exception as e:
            print(f"❌ خطأ في الحصول على سجل الدخل: {e}")
            return []
    ...
    def get_all_trades_data(self):
        ...
        for symbol in symbols:
            try:
                ...
                if trades:
                    ...
                    print(f"✅ تم جمع {len(trades)} صفقة لـ {symbol}")
        ...
        print(f"📊 إجمالي الصفقات المجمعة: {len(all_trades)}")
        return pd.DataFrame(all_trades) if all_trades else pd.DataFrame()

    def simulate_trade_data(self):
        """محاكاة بيانات الصفقات للاختبار"""
        ...
        for symbol in symbols:
            ...
            for i in range(np.random.randint(3, 8)):
                ...
                realized_pnl = np.random.uniform(-100, 200)
        ...
        print(f"🔧 تم إنشاء {len(trades)} صفقة محاكاة")
        return pd.DataFrame(trades)

    def analyze_trades(self, trades_df):
        """تحليل الصفقات"""
        if trades_df.empty:
            print("📭 لا توجد صفقات للتحليل")
            return pd.DataFrame()
        
        try:
            print("📈 جاري تحليل بيانات الصفقات...")
            ...
            print(f"📊 تحليل الصفقات: {initial_count} -> {final_count} صفقة بعد التنظيف")
    ...
    def calculate_pnl(self, trades_df):
        ...
        try:
            print("🧮 جاري حساب الربح والخسارة...")
            ...
            print(f"✅ تم حساب PnL لـ {len(results)} عملة")
    ...
    def create_summary_plot(self, pnl_df, positions_df):
        """إنشاء رسم بياني"""
        try:
            if pnl_df.empty and not positions_df:
                print("📭 لا توجد بيانات للرسم البياني")
                return None
            ...
            print("✅ تم إنشاء الرسم البياني بنجاح")
            return plot_base64
    ...
    async def generate_full_report(self):
        ...
        try:
            ...
            # محاولة جلب الصفقات الحقيقية أولاً
            trades_df = self.get_all_trades_data()
            
            # إذا لم توجد صفقات حقيقية، استخدم بيانات محاكاة
            if trades_df.empty:
                print("🔧 استخدام بيانات محاكاة للصفقات...")
                trades_df = self.simulate_trade_data()
