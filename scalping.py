...
class ScalpingSignalGenerator:
    ...
    def _calculate_indicators(self, data, current_price):
        ...
        # 3. فيبوناتشي (آخر 60 دقيقة - 12 شمعة 5 دقائق)
        high_60m = df['high'].rolling(12).max()
        low_60m = df['low'].rolling(12).min()
    ...
    def _analyze_long_signal(self, indicators):
        ...
        # 3. تأكيد فيبوناتشي (دعم)
        fib_618 = indicators['fib_levels']['fib_618']
        fib_786 = indicators['fib_levels']['fib_786']
        price_vs_fib = min(
            abs(indicators['current_price'] - fib_618) / fib_618,
            abs(indicators['current_price'] - fib_786) / fib_786
        )
        conditions.append(price_vs_fib < 0.005)  # ضمن 0.5% من مستويات فيبوناتشي
        
        # 4. زخم RSI
        conditions.append(indicators['rsi'] < 65)  # ليس في منطقة تشبع شراء
    ...
    def _analyze_short_signal(self, indicators):
        """تحليل إشارة البيع للسكالبينج"""
        conditions = []
        
        # 1. تقاطع المتوسطات (الاتجاه الهابط)
        conditions.append(indicators['ema5'] < indicators['ema10'])
        conditions.append(indicators['ema10'] < indicators['ema20'])
        
        # 2. موقع السعر من بولينجر باند (تشبع شراء)
        bb_position = (indicators['current_price'] - indicators['bb_lower']) / (indicators['bb_upper'] - indicators['bb_lower'])
        conditions.append(bb_position > 0.7)  # قريب من النطاق العلوي
        
        # 3. تأكيد فيبوناتشي (مقاومة)
        fib_236 = indicators['fib_levels']['fib_236']
        fib_382 = indicators['fib_levels']['fib_382']
        price_vs_fib = min(
            abs(indicators['current_price'] - fib_236) / fib_236,
            abs(indicators['current_price'] - fib_382) / fib_382
        )
        conditions.append(price_vs_fib < 0.005)  # ضمن 0.5% من مستويات فيبوناتشي
        
        # 4. زخم RSI
        conditions.append(indicators['rsi'] > 35)  # ليس في منطقة تشبع بيع
...
class ScalpingTradingBot:
    ...
    TRADING_SETTINGS = {
        ...
        'used_balance_per_trade': 6,  # ⬅️ الرصيد المستخدم: 6$
        'max_leverage': 4,            # ⬅️ الرافعة: 4x
        'nominal_trade_size': 24,     # ⬅️ الرصيد الاسمي: 24$ (6$ × 4)
        'max_active_trades': 2,
        'data_interval': '3m',        # ⬅️ إطار زمني للسكالبينج
        'rescan_interval_minutes': 2, # ⬅️ مسح أسرع
        'min_signal_confidence': 0.70,
        'target_profit_pct': 0.15,    # ⬅️ هدف 0.15%
        'stop_loss_pct': 0.10,        # ⬅️ وقف 0.10%
        'max_daily_trades': 30,
        'cooldown_after_loss': 5,     # ⬅️ تبريد بعد خسارة (دقائق)
    }
    ...
    def __init__(self):
        ...
        logger.info("✅ تم تهيئة بوت السكالبينج بنجاح")
    ...
    def send_performance_report(self):
        ...
        message = (
            ...
            f"الرصيد الحالي: ${self.performance_stats.get('current_balance', 0):.2f}\n"
            f"الوقت: {datetime.now(damascus_tz).strftime('%H:%M:%S')}"
        )
        self.notifier.send_message(message)
    ...
    def run(self):
        """تشغيل البوت الرئيسي"""
        logger.info("🚀 بدء تشغيل بوت السكالبينج...")
