...
class ScalpingTradingBot:
    ...
    def send_performance_report(self):
        ...
        message = (
            f"📊 <b>تقرير أداء السكالبينج المحسن (بدون حدود)</b>\n"
    ...
    def send_balance_report(self):
        ...
        try:
            ...
            message = (
                f"💰 <b>تقرير الرصيد الحقيقي (بدون حدود)</b>\n"
                f"الجلسة: {self.dynamic_settings['session_name']}\n"
                f"الرصيد الإجمالي: ${balance['total_balance']:.2f}\n"
    ...
    def send_session_report(self):
        ...
        message = (
            f"🌍 <b>تقرير جلسة التداول المحسن (بدون حدود)</b>\n"
            ...
            f"الشروط: {'مخففة 🔓' if self.dynamic_settings['relaxed_conditions'] else 'مشددة 🔒'}\n"
            f"الحدود: غير محدود ✅\n"
    ...
    def send_heartbeat(self):
        ...
        if self.notifier:
            active_trades = self.trade_manager.get_active_trades_count()
            message = f"💓 بوت السكالبينج المحسن (بدون حدود) نشط - الجلسة: {self.dynamic_settings['session_name']} - الصفقات: {active_trades}"
            self.notifier.send_message(message)
        ...
        try:
            ...
            if order and order['orderId']:
                ...
                trade_data = {
                    ...
                    'relaxed_conditions': signal.get('relaxed_conditions', False)
                }
                ...
                if self.notifier:
                    ...
                    message = (
                        f"{'🟢' if direction == 'LONG' else '🔴'} <b>فتح صفقة سكالبينج{condition_type}</b>\n"
                        f"الجلسة: {self.dynamic_settings['session_name']}\n"
                        f"العملة: {symbol}\n"
                        f"الاتجاه: {direction}\n"
        ...
        return [
            {
                ...
                'relaxed_conditions': trade.get('relaxed_conditions', False)
            }
