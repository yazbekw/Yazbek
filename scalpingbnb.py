...
TRADING_SETTINGS = {
    'symbols': ["BNBUSDT"],
    'used_balance_per_trade': 10,
    'max_leverage': 5,
    'nominal_trade_size': 50,
    'max_active_trades': 2,
    'data_interval': '5m',
    'rescan_interval_minutes': 3,
    'min_signal_confidence': 0.80,  # ⬅️ تم تخفيضها لتناسب المؤشرات الإضافية
    'target_profit_pct': 0.15,
    'stop_loss_pct': 0.08,
    'max_trade_duration_minutes': 20,
    'max_daily_trades': 30,
    'cooldown_after_loss': 5,
    'max_trades_per_symbol': 2,  # ⬅️ إضافة جديدة: الحد الأقصى للصفقات على نفس العملة
}
...
class ScalpingSignalGenerator:
    ...
    def _calculate_indicators(self, data, current_price):
        ...
        # مؤشر الماكد (MACD) - الإضافة الجديدة
    ...
    def _analyze_long_signal(self, indicators):
        ...
        weights.append(0.20)  # ⬅️ وزن مخفّض
        
        conditions.append(indicators['ema10'] > indicators['ema20'])
        weights.append(0.25)  # ⬅️ وزن مخفّض
        ...
        # شروط الماكد الجديدة - الإضافة الرئيسية
        conditions.append(indicators['macd_above_signal'])  # الماكد فوق خط الإشارة
        weights.append(0.15)
        
        conditions.append(indicators['macd_histogram_positive'])  # هيسطجرام موجب
        weights.append(0.10)
        
        conditions.append(indicators.get('macd_trend_up', False))  # اتجاه الماكد صاعد
        weights.append(0.05)
        
        conditions.append(indicators.get('macd_histogram_increasing', False))  # هيسطجرام متزايد
        weights.append(0.05)
    ...
    def _analyze_short_signal(self, indicators):
        ...
        # شروط الماكد الجديدة - الإضافة الرئيسية
        conditions.append(not indicators['macd_above_signal'])  # الماكد تحت خط الإشارة
        weights.append(0.15)
        
        conditions.append(not indicators['macd_histogram_positive'])  # هيسطجرام سالب
        weights.append(0.10)
        
        conditions.append(not indicators.get('macd_trend_up', True))  # اتجاه الماكد هابط
        weights.append(0.05)
        
        conditions.append(not indicators.get('macd_histogram_increasing', True))  # هيسطجرام متناقص
        weights.append(0.05)
...
class ScalpingTradingBot:
    ...
    def __init__(self):
        ...
        logger.info("✅ تم تهيئة بوت السكالبينج بنجاح مع إضافة مؤشر الماكد")
    ...
    def send_performance_report(self):
        ...
        message = (
            f"📊 <b>تقرير أداء السكالبينج المحسّن</b>\n"
            f"الاستراتيجية: تقاطع المتوسطات + RSI + الماكد\n"
    ...
    def send_heartbeat(self):
        if self.notifier:
            active_trades = self.trade_manager.get_active_trades_count()
            message = f"💓 بوت السكالبينج المحسّن نشط - الصفقات النشطة: {active_trades}"
            self.notifier.send_message(message)
    ...
    def can_open_trade(self, symbol, direction):
        ...
        # التحقق من إمكانية فتح صفقة جديدة على نفس العملة
        ...
        # التحقق من فترة التبريد بعد الخسائر المتتالية
    ...
    def execute_trade(self, signal):
        try:
            ...
            if order and order['orderId']:
                ...
                if self.notifier:
                    # الحصول على معلومات الصفقات الحالية على هذه العملة
    ...
    def scan_market(self):
        logger.info("🔍 بدء مسح السوق للسكالبينج المحسّن...")
        
        opportunities = []
        
        for symbol in self.TRADING_SETTINGS['symbols']:
            try:
                # السماح بمسح السوق حتى لو كانت هناك صفقات نشطة على العملة
                # سيتم التحقق من إمكانية فتح صفقة جديدة في دالة can_open_trade
    ...
    def run(self):
        logger.info("🚀 بدء تشغيل بوت السكالبينج المحسّن...")
