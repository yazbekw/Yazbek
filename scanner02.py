...
INDICATOR_WEIGHTS = {
    ...
    "PRICE_ACTION": 20,       # أنماط الشموع + اختراق المتوسطات
    "KEY_LEVELS": 20,         # الدعم/المقاومة + فيبوناتشي
    "VOLUME_CONFIRMATION": 15, # تحليل الحجم
    "BLOCKCHAIN_HEALTH": 10,   # BCMI (إذا مفعل)
    "DIVERGENCE_SIGNALS": 10   # التباعد (إذا مفعل)
}
...
class LRUCache:
    """ذاكرة تخزين مؤقت مع آلية LRU لمنع تسرب الذاكرة"""
...
class AdvancedMarketAnalyzer:
    ...
    def calculate_rsi(prices: List[float], period: int = 14) -> float:
        ...
        try:
            ...
            if avg_losses[-1] == 0:
                return 100.0 if avg_gains[-1] > 0 else 50.0
    ...
    def detect_candle_pattern(prices: List[float], highs: List[float], lows: List[float]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"pattern": "none", "strength": 0, "description": "⚪ خطأ في تحليل النمط", "direction": "none"}
    ...
    def analyze_volume_trend(volumes: List[float], price_trend: str = "neutral") -> Dict[str, Any]:
        ...
        try:
            ...
            return {"trend": "stable", "strength": 0, "description": "⚪ خطأ في تحليل الحجم", "volume_ratio": 1.0}
...
• 💾 *حجم الكاش:* `{data_fetcher.cache.size()}` عملة
...
class ExecutorBotClient:
    """عميل للتواصل مع بوت التنفيذ - مع تحسينات إدارة الموارد"""
    ...
    def _initialize_client(self):
        """تهيئة عميل HTTP مع إعدادات مناسبة"""
...
class BinanceDataFetcher:
    """جلب البيانات من Binance مع إدارة محسنة للذاكرة"""
    def __init__(self):
        self.client = None
    ...
    async def get_coin_data(self, coin_data: Dict[str, str], timeframe: str) -> Dict[str, Any]:
        ...
        if cached_data and current_time - cached_data['timestamp'] < 300:
            return cached_data['data']
        
        try:
            data = await self._fetch_binance_data_with_retry(coin_data['binance_symbol'], timeframe)
            
            if not data.get('prices') or len(data['prices']) < 10:
                safe_log_error(f"فشل جلب بيانات {timeframe} لـ {coin_data['symbol']}", 
                             coin_data['symbol'], "data_fetcher")
                return self._get_fallback_data(coin_data, timeframe)
    ...
    async def _fetch_binance_data_with_retry(self, symbol: str, interval: str, retries: int = 3) -> Dict[str, List[float]]:
        """جلب البيانات مع إعادة المحاولة"""
        for attempt in range(retries):
            try:
                data = await self._fetch_binance_data(symbol, interval)
                if data['prices'] and len(data['prices']) >= 10:
                    return data
                else:
                    safe_log_error(f"بيانات غير كافية للمحاولة {attempt + 1}", symbol, "binance")
            except Exception as e:
                safe_log_error(f"فشل المحاولة {attempt + 1}: {e}", symbol, "binance")
    ...
    async def _fetch_binance_data(self, symbol: str, interval: str) -> Dict[str, List[float]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                safe_log_error(f"استجابة غير ناجحة من Binance: {response.status_code}", symbol, "binance")
        except httpx.TimeoutException:
            safe_log_error("انتهت مهلة الاتصال بـ Binance", symbol, "binance")
        except Exception as e:
            safe_log_error(f"خطأ في جلب البيانات من Binance: {e}", symbol, "binance")
        
        return {'prices': [], 'highs': [], 'lows': [], 'volumes': []}

    def _get_fallback_data(self, coin_data: Dict, timeframe: str) -> Dict[str, Any]:
...
# =============================================================================
# تهيئة الكائنات الرئيسية - بعد تعريف جميع الفئات
# =============================================================================
...
async def robust_data_fetch(coin_data: Dict, timeframe: str, retries: int = 3) -> Optional[Dict[str, Any]]:
    """جلب بيانات قوي مع إعادة المحاولة"""
    for attempt in range(retries):
        try:
            ...
            safe_log_error(f"فشل محاولة جلب البيانات {attempt + 1}: {e}", coin_data['symbol'], "robust_fetch")
...
async def check_with_confirmation(coin_data: Dict) -> Optional[Dict[str, Any]]:
    ...
    if not ENABLE_CONFIRMATION_SYSTEM:
        ...
        primary_data = await robust_data_fetch(coin_data, PRIMARY_TIMEFRAME)
        if primary_data and primary_data['analysis']['strongest_score'] >= CONFIDENCE_THRESHOLD:
            return {
                **primary_data['analysis'],
                'price': primary_data['price'],
                'prices': primary_data.get('prices', []),
    ...
    try:
        ...
        confirmation_data = await robust_data_fetch(coin_data, CONFIRMATION_TIMEFRAME)
...
async def market_scanner_task():
    ...
    while True:
        try:
                    ...
                    if confirmed_analysis:
                        ...
                        if primary_data:
                            ...
                            await notifier.send_alert(
                                ...
                                primary_data['price'], primary_data.get('prices', []),
                                primary_data.get('highs', []), primary_data.get('lows', [])
                            )
            ...
            # حساب الوقت المتبقي للجولة التالية مع تعويض وقت التنفيذ
            end_time = datetime.now()
            execution_time = (end_time - start_time).total_seconds()
            sleep_time = max(0, SCAN_INTERVAL - execution_time)
            
            safe_log_info(f"انتهت جولة المسح - العملات: {len(SUPPORTED_COINS)} - الإشارات: {system_stats['total_signals_sent']} - الوقت المتبقي: {sleep_time:.1f} ثانية", 
                         "system", "scanner")
            
            await asyncio.sleep(sleep_time)
            
        except asyncio.CancelledError:
            safe_log_info("تم إلغاء مهمة المسح", "system", "scanner")
            break
        except Exception as e:
...
async def health_check_task():
    ...
    while True:
        try:
            ...
            current_time = time.time()
            cache_size = data_fetcher.cache.size()
            current_session = get_current_session()
            ...
            safe_log_info("تم إلغاء مهمة الفحص الصحي", "system", "health")
            break
        except Exception as e:
...
async def heartbeat_task():
    ...
    while True:
        try:
            ...
            safe_log_info("تم إلغاء مهمة النبضات", "system", "heartbeat")
            break
        except Exception as e:
...
async def executor_heartbeat_task():
    ...
    while True:
        try:
            ...
            safe_log_info("تم إلغاء مهمة نبضات المنفذ", "system", "executor_heartbeat")
            break
        except Exception as e:
...
async def scan_coin(coin: str, timeframe: str = "15m"):
    ...
    if not coin.isalpha() or len(coin) > 10:
        raise HTTPException(400, "اسم العملة غير صالح")
...
async def test_executor_connection():
    ...
    try:
        ...
        return {
            ...
            "executor_url": "مضبوط" if EXECUTOR_BOT_URL else "غير مضبوط",
            "trade_execution_enabled": EXECUTE_TRADES,
...
async def get_cache_info():
    ...
    return {
        "cache_size": data_fetcher.cache.size(),
        "max_cache_size": data_fetcher.cache.max_size,
        "cache_usage_percent": (data_fetcher.cache.size() / data_fetcher.cache.max_size) * 100
    }
...
async def clear_cache():
    """مسح الذاكرة المؤقتة"""
    try:
        old_size = data_fetcher.cache.size()
        data_fetcher.cache.clear()
        return {
            "status": "success",
            "message": f"تم مسح الذاكرة المؤقتة ({old_size} عنصر)",
            "new_size": data_fetcher.cache.size()
        }
    except Exception as e:
        return {"status": "error", "message": str(e)}
...
async def startup_event():
    safe_log_info("بدء تشغيل ماسح القمم والقيعان الإصدار 3.0", "system", "startup")
    ...
    safe_log_info(f"رابط البوت المنفذ: {'مضبوط' if EXECUTOR_BOT_URL else 'غير مضبوط'}", "system", "config")
...
async def shutdown_event():
    ...
    safe_log_info("✅ تم إغلاق جميع الموارد", "system", "shutdown")
