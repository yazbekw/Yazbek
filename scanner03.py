...
# إعدادات البوت المنفذ - إضافة جديدة
...
# إعدادات تصفية الإشارات المتضاربة - جديد
MIN_SIGNAL_GAP = 15                 # فرق 15 نقطة على الأقل بين القمة والقاع
CONFLICTING_SIGNAL_PENALTY = 20     # تخفيض نقاط الإشارة المضادة عند التضارب
...
app = FastAPI(title="Crypto Top/Bottom Scanner", version="2.3.0")
...
system_stats = {
    ...
    "conflicting_signals_filtered": 0  # جديد: عدد الإشارات المتضاربة التي تم تصفيتها
}
...
def filter_conflicting_signals(analysis: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # إذا كان الفرق كافياً، نعيد الإشارة الأقوى فقط
...
class AdvancedMarketAnalyzer:
    """محلل متقدم للقمم والقيعان - النسخة المحدثة"""
    ...
    def detect_candle_pattern(prices: List[float], highs: List[float], lows: List[float]) -> Dict[str, Any]:
        """الكشف عن أنماط الشموع الانعكاسية - محدث"""
        if len(prices) < 3:
    ...
    def analyze_support_resistance(prices: List[float]) -> Dict[str, Any]:
        """تحليل مستويات الدعم والمقاومة - محدث"""
        if len(prices) < 20:
    ...
    def analyze_volume_trend(volumes: List[float], price_trend: str = "neutral") -> Dict[str, Any]:
        ...
        if volume_ratio > 2.0:
            ...
            strength = 6  # حجم منخفض عند نقاط الانعكاس
    ...
    def analyze_market_condition(self, prices: List[float], volumes: List[float], 
        ...
        """تحليل شامل لحالة السوق للكشف عن القمم والقيعان - النسخة المحدثة"""
    ...
    def _calculate_top_score(self, rsi: float, stoch: Dict, macd: Dict, moving_averages: Dict,
        ...
        """حساب نقاط القمة - النسخة المحدثة"""
        score = 0
    ...
    def _calculate_bottom_score(self, rsi: float, stoch: Dict, macd: Dict, moving_averages: Dict,
                              candle_pattern: Dict, support_resistance: Dict,
                              volume_analysis: Dict, fib_levels: Dict, current_price: float) -> int:
        """حساب نقاط القاع - النسخة المحدثة"""
        score = 0
...
class TelegramNotifier:
    ...
    async def send_heartbeat(self) -> bool:
        ...
        try:
            ...
            message = f"""
💓 *نبضة النظام - ماسح القمم والقيعان v2.3*
...
✅ *الحالة:* النظام يعمل بشكل طبيعي مع تصفية الإشارات المتضاربة
            """
    ...
    def _build_beautiful_message(self, coin: str, timeframe: str, analysis: Dict[str, Any], price: float) -> str:
        ...
        # التوقيع
        message += "─" * 30 + "\n"
        message += f"⚡ *ماسح القمم والقيعان v2.3*"
...
class ExecutorBotClient:
    """عميل للتواصل مع بوت التنفيذ - محدث"""
    ...
    async def send_trade_signal(self, signal_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            payload = {
                "signal": signal_data,
                "timestamp": time.time(),
                "source": "top_bottom_scanner_v2.3"
            }
...
async def check_with_confirmation(coin_data):
    """فحص الإشارة مع التأكيد من إطار 15m باستخدام التحليل المحسن"""
    try:
        # 1. الفحص في الإطار الرئيسي (1h) باستخدام التحليل المحسن
...
async def market_scanner_task():
    """المهمة الرئيسية للمسح الضوئي مع نظام التأكيد وتحسينات تجنب الإشارات المتضاربة"""
    safe_log_info("بدء مهمة مسح السوق كل 20 دقيقة مع نظام التأكيد وتحسينات تجنب الإشارات المتضاربة", "system", "scanner")
            ...
            for coin_key, coin_data in SUPPORTED_COINS.items():
                try:
                    ...
                    if confirmed_signal:
                        ...
                        market_bias = get_market_bias(confirmed_signal['prices'])
                        confirmed_signal = apply_market_bias(confirmed_signal, market_bias)
                        
                        # 🔴 تطبيق تصفية الإشارات المتضاربة
                        filtered_signal = filter_conflicting_signals(confirmed_signal)
                        
                        if not filtered_signal:
                            continue  # تخطي إذا كانت الإشارات متضاربة
                            
                        if filtered_signal['alert_level']['send_alert']:
                            # إرسال التنبيه
...
async def health_check_task():
    ...
    while True:
        try:
            ...
            safe_log_info(f"الفحص الصحي - الكاش: {cache_size} - الجلسة: {current_session['name']} - الوزن: {current_session['weight']} - المنفذ: {'متصل' if executor_health else 'غير متصل'} - إشارات مفلترة: {system_stats['conflicting_signals_filtered']}", 
                         "system", "health")
...
async def test_telegram():
    ...
    try:
        test_message = """
🧪 *اختبار البوت - ماسح القمم والقيعان v2.3*

✅ *الحالة:* البوت يعمل بشكل صحيح مع تصفية الإشارات المتضاربة
🕒 *الوقت:* {}
🌍 *الجلسة:* {} {}
⚡ *الإصدار:* 2.3.0
...
🎯 *الوظيفة:* كشف القمم والقيعان تلقائياً مع تصفية الإشارات المتضاربة
        """.format(
...
async def startup_event():
    safe_log_info("بدء تشغيل ماسح القمم والقيعان الإصدار 2.3 مع تصفية الإشارات المتضاربة", "system", "startup")
    ...
    safe_log_info("✅ بدأت مهام المسح والفحص الصحي والنبضات مع تصفية الإشارات المتضاربة", "system", "startup")
