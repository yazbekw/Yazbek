...
CONFIDENCE_THRESHOLD = 40  # عتبة الثقة الأساسية
...
CONFLICT_MANAGEMENT = {
    ...
    "CONFLICT_RESOLUTION_STRATEGY": "enhanced"
}
...
app = FastAPI(title="Crypto Top/Bottom Scanner", version="3.0.0")
...
system_stats = {
    ...
    "data_source_used": "coinex"  # جديد: تتبع مصدر البيانات المستخدم
}
...
• 📡 *مصدر البيانات:* `{system_stats['data_source_used']}`
...
✅ *الحالة:* النظام يعمل بشكل طبيعي مع النظام المحسن لتقليل التضارب
    ...
    def _build_enhanced_message(self, coin: str, timeframe: str, analysis: Dict[str, Any], price: float) -> str:
        ...
        message += f"📡 *مصدر البيانات:* `{system_stats['data_source_used']}`\n\n"
...
class MultiSourceDataFetcher:
    """جلب البيانات من Coinex و Binance"""
    
    def __init__(self):
        self.client = httpx.AsyncClient(timeout=30.0)
        self.analyzer = AdvancedMarketAnalyzer()
        self.cache = {}
        self.current_source = "coinex"  # المصدر الافتراضي

    async def _fetch_coinex_data(self, symbol: str, interval: str) -> Dict[str, List[float]]:
        """جلب البيانات من Coinex"""
        try:
            # تحويل الإطار الزمني إلى تنسيق Coinex
            timeframe_map = {
                '15m': '15min',
                '1h': '1hour'
            }
            coinex_interval = timeframe_map.get(interval, interval)
            
            url = f"https://api.coinex.com/v1/market/kline?market={symbol}&type={coinex_interval}&limit=50"
            ...
            if response.status_code == 200:
                ...
                if data.get('code') == 0 and data.get('data'):
                    klines = data['data']
                    prices = [float(k[2]) for k in klines]  # سعر الإغلاق
                    highs = [float(k[3]) for k in klines]   # أعلى سعر
                    lows = [float(k[4]) for k in klines]    # أدنى سعر
                    volumes = [float(k[5]) for k in klines] # الحجم
            ...
            safe_log_error(f"فشل جلب البيانات من Coinex: {response.status_code}", symbol, "coinex")
            return {'prices': [], 'highs': [], 'lows': [], 'volumes': []}
            
        except Exception as e:
            safe_log_error(f"خطأ في جلب البيانات من Coinex: {e}", symbol, "coinex")
            return {'prices': [], 'highs': [], 'lows': [], 'volumes': []}
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data and len(data) > 0:
                    ...
                    volumes = [float(item[5]) for item in data] # الحجم
            ...
            safe_log_error(f"فشل جلب البيانات من Binance: {response.status_code}", symbol, "binance")
            return {'prices': [], 'highs': [], 'lows': [], 'volumes': []}
    ...
    async def get_coin_data(self, coin_data: Dict[str, str], timeframe: str) -> Dict[str, Any]:
        """جلب بيانات العملة من Coinex أولاً ثم Binance كبديل"""
        
        cache_key = f"{coin_data['symbol']}_{timeframe}"
        ...
        try:
            # المحاولة الأولى: Coinex
            data = await self._fetch_coinex_data(coin_data['coinex_symbol'], timeframe)
            source_used = "coinex"
            
            # إذا فشل Coinex، جرب Binance
            if not data.get('prices'):
                safe_log_info(f"🔁 التبديل إلى Binance لـ {coin_data['symbol']}", coin_data['symbol'], "data_fetcher")
                data = await self._fetch_binance_data(coin_data['binance_symbol'], timeframe)
                source_used = "binance"
            
            if not data.get('prices'):
                safe_log_error(f"فشل جلب بيانات {timeframe} لـ {coin_data['symbol']} من كلا المصدرين", 
                             coin_data['symbol'], "data_fetcher")
                return self._get_fallback_data()
            
            # تحديث مصدر البيانات المستخدم
            system_stats["data_source_used"] = source_used
            ...
            if analysis.get('breakdown'):
                ...
                safe_log_info(f"تم تحليل {coin_data['symbol']} ({timeframe}) - النظام المحسن - قمة: {analysis['top_score']} - قاع: {analysis['bottom_score']} - مصدر البيانات: {source_used}", 
                             coin_data['symbol'], "enhanced_analyzer")
            else:
                safe_log_info(f"تم تحليل {coin_data['symbol']} ({timeframe}) - قمة: {analysis['top_score']} - قاع: {analysis['bottom_score']} - مصدر البيانات: {source_used}", 
...
async def prepare_trade_signal(coin_key: str, coin_data: Dict, timeframe: str, 
    ...
    try:
        ...
        signal_data = {
            "coin": coin_key,
            "symbol": coin_data["binance_symbol"],  # استخدام رمز Binance للتنفيذ
            "action": action,
...
async def root():
    return {
        ...
        "current_data_source": system_stats["data_source_used"]
    }
...
async def get_system_stats():
    ...
    return {
        ...
        "trend_alignment_applied": system_stats["trend_alignment_applied"],
        "conflict_penalties_applied": system_stats["conflict_penalties_applied"],
        "data_source_used": system_stats["data_source_used"],
        "enhanced_system": {
            "conflict_management": CONFLICT_MANAGEMENT,
            "indicator_weights": ENHANCED_INDICATOR_WEIGHTS,
            "version": "3.0.0"
...
async def startup_event():
    ...
    safe_log_info(f"مصادر البيانات: Coinex (أساسي), Binance (احتياطي)", "system", "config")
